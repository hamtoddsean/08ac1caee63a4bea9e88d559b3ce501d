...
class Client(object):
    ...
    def aborta(self,aabcp,aabtral,aabplist,aabbid):
        abcdaa=''
        abcdaa=0.1/100*float(aabplist['buyp'][-1])
        abcdaa+=float(aabplist['buyp'][-1])
        asetamto=-500
        asetamtt=1000000
        atutto=aabcp[0]
        atuttw=aabtral[0]
        if aabcp[-1]< aabtral[-1]:
            atutto+=24*3600
        if atutto-atuttw>= 300:
            if float(aabbid)< abcdaa:
                self.totsell(aabplist,aabbid,asetamto,asetamtt)
        
    def groupsella(self, agplist, agbid):
        agroclie='ETHUSD'
        if agplist['status']=='sell':
            agp= self.gpricep(1000)
            acpg= self.reedt(agp)
            atdstamp=self.tradehistory(agroclie)
            atradestamp= self.tradestampbuy(atdstamp)
            atralg= self.reedt(atradestamp)
            self.aborta(acpg,atralg,agplist,agbid)
...
#########
    ...
    def totbuy(self,tutplist):
        ...
        tutplist['buych'].append(float(csppp))
    ...
    def totsell(self,totsplist,totsbid,tssetamt,tssetamtt):
        ...
        psprc*=sprc
        psprc=float(psprc)
        zprc= float(bsppp)-totsplist['buych'][-1]
        zzprc=buyper/zprc
        zzprc*=100
        zzprc=float(zzprc)
        #totsplist['sellch'].append(float(bsppp))
...
if __name__ == "__main__":
    ...
    clist={'buych':[],'sellch':[],'buyper':[],'keeprecord':[],'go':'','qty':'0.007','pleven':'','ggo':'yes','list':[],'buyp':[],'sellp':[],'BUY':[],'SELL':[],'even':'','count':[],'status':'','buy':[],'sell':[],'lower':[],'lowert':[],'uppert':[],'upper':[],'w':[],'ww':[],'counter':[],'store':[],'tab':['d'],'watch':''}
